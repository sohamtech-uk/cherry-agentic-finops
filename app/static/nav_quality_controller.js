(() => {
  ...
  function mount() {
    ...
    anchor.insertAdjacentHTML(
      "afterend",
      `<section class="navqc-shell" id="nav-quality-controller">
        <div class="navqc-nav">
          <a href="#fund-manager">General Control Review</a>
          <a class="active" href="#nav-quality-controller">NAV Quality Controller</a>
        </div>
        <div class="navqc-card"><div id="navqc-content"></div></div>
      </section>`,
    );
    render();
  }
  ...
  function targetBanner(target) {
    if (!target) return "";
    return `<div class="navqc-banner"><div><span class="eyebrow">Transcript baseline</span>
      <strong>${esc(target.transcript_baseline)}</strong></div><div><span class="eyebrow">Product target</span>
      <strong>${esc(target.target)}</strong></div><div><span class="eyebrow">Measured by</span>
      <strong>${esc(target.measurement)}</strong></div></div>
      <div class="navqc-boundary">${esc(target.claim_boundary)}</div>`;
  }

  function header() {
    return `<div class="navqc-head"><div><p class="eyebrow">Fund Manager · specialist workflow</p>
      <h2>NAV Quality Controller</h2><p>Find the full NAV exception set in one review, consolidate the administrator remediation package, and measure whether actual review rounds fall from the transcript baseline toward 1–2.</p></div>
  ...
  function noCase() {
    return `<div class="navqc-panel"><h3>Start with the General Control Review</h3>
      <div class="navqc-empty">Upload and classify evidence in the General Control Review first. The NAV Quality Controller reuses that case and its stored evidence.</div>
  ...
  function evidence() {
    ...
    const navEvidence = sources.filter((source) =>
      ["nav_workbook", "investor_gl", "side_letter", "lpa"].includes(source.detected_type),
    );
  ...
  function readinessView(readiness) {
    ...
    return `<div class="navqc-panel"><h3>NAV readiness</h3><p>Checks what can run from the evidence already held on this Fund Manager case.</p>
      ${targetBanner(readiness.round_reduction_target)}
  ...
  function reconciliationView(result) {
    ...
    return `<div class="navqc-panel"><h3>NAV reconciliation</h3><p>The existing deterministic NAV Quality Controller performed the financial checks.</p>
      ...
      <div class="navqc-metrics"><div><span>Controls passed</span><strong>${esc(review.controls_passed ?? 0)}</strong></div><div><span>Exceptions</span><strong>${esc(review.exceptions_open ?? findings.length)}</strong></div><div><span>Actual round</span><strong>${esc(result.iteration?.round_number ?? 1)}</strong></div></div></div>
      ${targetBanner(result.round_reduction_target)}
  ...
  function remediationSummary(review) {
    const pack = review.remediation_package;
    if (!pack) return "";
    return `<div class="navqc-banner"><div><span class="eyebrow">Consolidated return package</span><strong>First-pass complete view</strong></div>
      <div class="navqc-metrics"><div><span>Findings</span><strong>${esc(pack.finding_count ?? 0)}</strong></div><div><span>Root causes</span><strong>${esc(pack.root_cause_count ?? 0)}</strong></div><div><span>Actions</span><strong>${esc(pack.work_item_count ?? 0)}</strong></div></div></div>
  ...
  function reviewView(review) {
    ...
    return `<div class="navqc-panel"><h3>Agentic NAV review</h3><p>${esc(review.agent_summary || "The Fund Manager investigation agent reviewed the deterministic NAV findings.")}</p>
      ...
      ${targetBanner(review.round_reduction_target)}
  ...
  function decisionForm(recommendation) {
    return `<div class="navqc-decision"><p>Agent recommendation: <strong>${esc(String(recommendation || "review").replaceAll("_", " "))}</strong></p>
      ...
      <div class="navqc-actions"><button class="navqc-button primary" data-nav-decision="approve_nav">Approve NAV</button><button class="navqc-button secondary" data-nav-decision="approve_with_exception">Approve with exception</button><button class="navqc-button secondary" data-nav-decision="request_evidence">Request evidence</button><button class="navqc-button secondary" data-nav-decision="return_to_administrator">Return complete package to administrator</button><button class="navqc-button danger" data-nav-decision="escalate">Escalate</button></div></div>`;
  }
  ...
  function historyView() {
    ...
    if (!state.history.available) {
      return `<div class="navqc-panel"><h3>Review history</h3><div class="navqc-empty">${esc(state.history.reason)}</div>${targetBanner(state.history.round_reduction_target)}</div>`;
    }
    const history = state.history.history || {};
    const rounds = history.history || history.rounds || history.iterations || [];
    return `<div class="navqc-panel"><h3>Review history</h3><p>${esc(history.legal_entity || "Fund")} · ${esc(history.period_end || "")}</p>
      ${targetBanner(state.history.round_reduction_target)}
      <div class="navqc-banner"><div><span class="eyebrow">Rounds submitted</span><strong>${esc(history.rounds_submitted ?? rounds.length)}</strong></div><div><span class="eyebrow">Rounds to close</span><strong>${esc(history.rounds_to_close ?? "Open")}</strong></div></div>
      <div class="navqc-history">${rounds.map((round) => `<div class="navqc-round"><span>Round ${esc(round.round_number)}</span><strong>${esc(String(round.action || "review").replaceAll("_", " "))}</strong><span>${esc(round.exceptions_open ?? 0)} exceptions</span></div>`).join("") || '<div class="navqc-empty">History is recorded but no round details were returned.</div>'}</div></div>`;
  }
  ...
  async function loadHistory() {
    ...
    try {
      state.history = await api(
        `/api/fund-manager/cases/${encodeURIComponent(state.caseData.case_id)}/nav/history`,
      );
      render();
    } catch (error) {
      notify(error.message || "Could not load NAV history.", true);
    }
  }

  function bind() {
    const id = state.caseData?.case_id;
    document.querySelector("#navqc-readiness")?.addEventListener(
      "click",
      () => run(`/api/fund-manager/cases/${encodeURIComponent(id)}/nav/readiness`),
    );
    document.querySelector("#navqc-refresh")?.addEventListener(
      "click",
      () => run(`/api/fund-manager/cases/${encodeURIComponent(id)}/nav/readiness`),
    );
    document.querySelector("#navqc-reconcile")?.addEventListener(
      "click",
      () => run(`/api/fund-manager/cases/${encodeURIComponent(id)}/nav/reconcile`),
    );
    document.querySelector("#navqc-review")?.addEventListener(
      "click",
